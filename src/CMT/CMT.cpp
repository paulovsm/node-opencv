...
NAN_METHOD(CMT::cmtTrack){
  SETUP_FUNCTION(CMT)
  //CMT *cmt = Nan::ObjectWrap::Unwrap<CMT>(info.This());
  ...
  v8::Local<v8::Array> arr = Nan::New<Array>(6);

  arr->Set(0, Nan::New<Number>((double) r.x));
  arr->Set(1, Nan::New<Number>((double) r.y));
  arr->Set(2, Nan::New<Number>((double) r.width));
  arr->Set(3, Nan::New<Number>((double) r.height));
  arr->Set(4, Nan::New<Number>((double) self->oCenter.x));
  arr->Set(5, Nan::New<Number>((double) self->oCenter.y));
